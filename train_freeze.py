...
def parse_args():
    ...
    parser.add_argument('--freeze_g_until_layer', type=int, default=None)
    parser.add_argument('--freeze_d_until_layer', type=int, default=None)
...
def main(args):
    ...
    if args.resume == 'GD':
        ...
        try:
            ...
            if args.freeze_g_until_layer is not None:
                print("Freezing Generator")
                for i, layer in enumerate(G.children()):
                    if i <= args.freeze_g_until_layer:
                        for param in layer.parameters():
                            param.requires_grad = False
                print("Generator Freezed")

            if args.freeze_d_until_layer is not None:
                print("Freezing Discriminator")
                for i, layers in enumerate(D.children()):
                    for j, layer in enumerate(layers):
                      if j <= args.freeze_d_until_layer:
                          for param in layer.parameters():
                              param.requires_grad = False
                print("Descriminator Freezed")
